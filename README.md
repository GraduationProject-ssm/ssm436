...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1t58veLEnL?p=34)


# 绪论
## 1.1 研究背景
当前社会各行业领域竞争压力非常大，随着当前时代的信息化，科学化发展，让社会各行业领域都争相使用新的信息技术，对行业内的各种相关数据进行科学化，规范化管理。这样的大环境让那些止步不前，不接受信息改革带来的信息技术的企业随时面临被淘汰，被取代的风险。所以当今，各个行业领域，不管是传统的教育行业，餐饮行业，还是旅游行业，医疗行业等领域都将使用新的信息技术进行信息革命，改变传统的纸质化，需要人手工处理工作事务的办公环境。软件信息技术能够覆盖社会各行业领域是时代的发展要求，各种数据以及文件真正实现电子化是信息社会发展的不可逆转的必然趋势。本农副产品销售网站也是紧跟科学技术的发展，运用当今一流的软件技术实现软件系统的开发，让医生管理信息完全通过管理系统实现科学化，规范化，程序化管理。从而帮助信息管理者节省事务处理的时间，降低数据处理的错误率，对于基础数据的管理水平可以起到促进作用，也从一定程度上对随意的业务管理工作进行了避免，同时，农副产品销售网站的数据库里面存储的各种动态信息，也为上层管理人员作出重大决策提供了大量的事实依据。总之，农副产品销售网站是一款可以真正提升管理者的办公效率的软件系统。
## 1.2 目的和意义
信息数据的处理完全依赖人工进行操作，会耗费大量的人工成本，特别是面对大量的数据信息时，传统人工操作不仅不能对数据的出错率进行保证，还容易出现各种信息资源的低利用率与低安全性问题。更有甚者，耽误大量的宝贵时间，尤其是对信息的更新，归纳与统计更是耗财耗力的过程。所以电子化信息管理的出现就能缓解以及改变传统人工方式面临的处境，一方面可以确保信息数据在短时间被高效处理，还能节省人力成本，另一方面可以确保信息数据的安全性，可靠性，并可以实现信息数据的快速检索与修改操作，这些优点是之前的旧操作模式无法比拟的。因此农副产品销售网站为数据信息的管理模式的升级与改革提供了重要的窗口。
## 1.3 论文结构安排
...
# 4 系统设计
农副产品销售网站的设计方案比如功能框架的设计，比如数据库的设计的好坏也就决定了该系统在开发层面是否高效，以及在系统维护层面是否容易维护和升级，因为在系统实现阶段是需要考虑用户的所有需求，要是在设计阶段没有经过全方位考虑，那么系统实现的部分也就无从下手，所以系统设计部分也是至关重要的一个环节，只有根据用户需求进行细致全面的考虑，才有希望开发出功能健全稳定的程序软件。
## 4.1 系统概要设计
...
|10|content|String|评论内容|是|
|11|reply|String|回复内容|是|
表4.3交流回复表
...
|30|content|String|留言内容|是|
|31|reply|String|回复内容|是|
表4.6农业资讯表
...
|34|introduction|String|简介|是|
|35|picture|String|图片|是|
|36|content|String|内容|是|
表4.7农业技巧表
...
|38|biaoti|String|标题|是|
|39|tupian|String|图片|是|
|40|jianshu|String|简述|是|
|41|shipin|String|视频|是|
|42|jiqiaoneirong|String|技巧内容|是|
|43|fariqi|date|发日期|是|
表4.8农业行情表
...
|45|biaoti|String|标题|是|
|46|tupian|String|图片|是|
|47|nongyexingqing|String|农业行情|是|
|48|faburiqi|datetime|发布日期|是|
表4.9商家表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|49|addtime|Date|创建时间|是|
|50|zhanghao|String|账号|是|
|51|mima|String|密码|是|
|52|shangjiaxingming|String|商家姓名|是|
|53|xingbie|String|性别|是|
|54|touxiang|String|头像|是|
|55|shenfenzheng|String|身份证|是|
|56|lianxidianhua|String|联系电话|是|
表4.10商品购买表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|57|addtime|Date|创建时间|是|
|58|dingdanbianhao|String|订单编号|是|
|59|shangpinmingcheng|String|商品名称|是|
|60|shangpinleixing|String|商品类型|是|
|61|zhanghao|String|账号|是|
|62|jiage|Integer|价格|是|
|63|shuliang|Integer|数量|是|
|64|zongjiage|String|总价格|是|
|65|goumaiburiqi|date|购买部日期|是|
|66|yonghuming|String|用户名|是|
|67|xingming|String|姓名|是|
|68|shouji|String|手机|是|
|69|dizhi|String|地址|是|
|70|ispay|String|是否支付|是|
表4.11商品类型表
...
|72|shangpinleixing|String|商品类型|是|
表4.12商品信息表
...
|74|shangpinmingcheng|String|商品名称|是|
|75|shangpinleixing|String|商品类型|是|
|76|tupian|String|图片|是|
|77|jiage|Integer|价格|是|
|78|shuliang|Integer|数量|是|
|79|faburiqi|date|发布日期|是|
|80|shangpinxiangqing|String|商品详情|是|
|81|zhanghao|String|账号|是|
|82|shangjiaxingming|String|商家姓名|是|
|83|lianxidianhua|String|联系电话|是|
|84|clicktime|datetime|最近点击时间|是|
|85|clicknum|Integer|点击次数|是|
表4.13收藏表表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|86|addtime|Date|创建时间|是|
|87|userid|Integer|用户id|是|
|88|refid|Integer|收藏id|是|
|89|tablename|String|表名|是|
|90|name|String|收藏名称|是|
|91|picture|String|收藏图片|是|
表4.14用户表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|92|addtime|Date|创建时间|是|
|93|yonghuming|String|用户名|是|
|94|mima|String|密码|是|
|95|xingming|String|姓名|是|
|96|xingbie|String|性别|是|
|97|touxiang|String|头像|是|
|98|shenfenzheng|String|身份证|是|
|99|shouji|String|手机|是|
表4.15用户表表
...
图5.3 商品类型管理页面
### 5.1农业资讯管理
如图5.4显示的就是农业资讯管理页面，此页面提供给管理员的功能有：根据农业资讯进行新增、修改、查询操作等等。
